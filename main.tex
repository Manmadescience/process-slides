...
\begin{frame}{Code Review}
	\begin{itemize}
		...
		\item Kann z.\,B. durch Scrum abgedeckt werden
	\end{itemize}
\end{frame}
...
\begin{frame}{Tests}
	\begin{itemize}
		\item Aktuell (\texttt{nosetests --with-coverage}):
		\item[]
			\begin{tabular}{@{}llll@{}}
			\toprule
			...
			\bottomrule
			\end{tabular}
		\item[] \texttt{Ran 53 tests in 20.421s}
		\item[] \texttt{FAILED (SKIP=1, errors=8)}
		\item Gar nicht mal so "ubel
		\item Integration Tests (wenn der Build-Prozess steht)
	\end{itemize}
\end{frame}
